...
public class FuelStationPump : MonoBehaviour
{
    ...
    [Tooltip("Price in TL per litre of fuel")]
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (playerBalance <= 0)
        {
            if (InteractionPromptHUD.Instance != null)
            {
                InteractionPromptHUD.Instance.ShowPrompt($"<color=#FF5555>⛽ [YETERSİZ BAKİYE]</color> Yakıt için para gerekli ({pricePerLiter} TL/L)");
            }
            SetPumpLightActive(false);
            return;
        }
        ...
        if (isHoldingRefuelKey)
        {
            ...
            if (InteractionPromptHUD.Instance != null)
            {
                ...
                InteractionPromptHUD.Instance.ShowPrompt($"<color=#32FFFF>⛽ YAKIT DOLUYOR...</color> {vehicle.currentFuel:F1} / {vehicle.maxFuel:F1} L (%{percent:F0}) - {pricePerLiter} TL/L");
            }
        }
        else
        {
            ...
            if (InteractionPromptHUD.Instance != null)
            {
                InteractionPromptHUD.Instance.ShowPrompt($"<b>{stationName}</b>: [F] Basılı Tut ➔ <b>Yakıt Doldur</b> ({pricePerLiter} TL/L)");
            }
        }
    }
